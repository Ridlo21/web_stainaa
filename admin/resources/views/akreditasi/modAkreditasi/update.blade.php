...
<?php
    $data = DB::table('landing_akreditasi')->where('id_landing_akreditasi', $id)->first();
?>
...
@section('konten')
    <section class="container-fluid p-4">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-12">
                <form id="formakreditasi" alamat="{{ route('akreditasi.modUpdate') }}" data-parsley-validate method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{$data->id_landing_akreditasi}}">
                            ...
                            <div class="row mb-2 align-items-center">
                                ...
                                <div class="col-sm-10">
                                    <div class="form-group">
                                        ...
                                        <?php
                                            if($data->gambar!=""){
                                                ...
                                                <div id="ed_priview3" class="col-md-12 mb-3 p-1 border" style="height:150px;">
                                                    <div  id="ed_pr_img3" style="background-repeat: no-repeat;background-position: left;background-size: contain;background-image:url('<?php echo asset('image')."/akreditasi"."/".$data->gambar ?>');width:100%;height:100%;">
                                                    </div>
                                                </div>
                                                <?php
                                            }else{
                                        ...
                                        <input class="form-control" type="file"  name="gambarakreditasi" id="gambarakreditasi"  <?= $gambar?>>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swal.fire({
                                ...
                                $('#spinnerWrapper').css('display', 'flex')
                                window.location.href = "{{ url('/akreditasiModIndex') }}"
                            })
                        }
                    })
                }
        ...
        $(function () {
            ...
            $("#gambarakreditasi").change(function(){
                if (this.files && this.files[0]) {
                    var size = this.files[0].size;
                    if (size > 100000000) {
                        alert("Maximum file size 1000px exceeds");
                        $("#gambarakreditasi").val("")
                        return;
                    } else {
                        if (this.files && this.files[0]) {
