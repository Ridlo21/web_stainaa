...
@endsection
<?php
    $data = DB::table('tentang_misi')->where('id_tentang_misi', $id)->first();
?>
@section('konten')
    <section class="container-fluid p-4">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-12">
                <form id="formMisi" alamat="{{ route('tentang.misiUpdate') }}" data-parsley-validate method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="{{$data->id_tentang_misi}}">
                    @csrf
                    {{-- {{ csrf_field() }} --}}
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row mb-2 align-items-center">
                                <label for="context" class="col-sm-2 col-form-label">Misi</label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" required name="misi" id="context" cols="50" rows="5"
                                        placeholder="Misi Stainaa">{{$data->isi_misi}}</textarea>
                                </div>
                            </div>
                        </div>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swal.fire({
                                ...
                                $('#spinnerWrapper').css('display', 'flex')
                                window.location.href = "{{ url('/tentangMisiIndex') }}"
                            })
                        }
                    })
                }
