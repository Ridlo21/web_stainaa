@extends('template')
@section('title')
    Tentang | Motto Tambah
@endsection
<?php
    $data = DB::table('tentang_motto')->where('id_tentang_motto', $id)->first();
?>
@section('konten')
    <section class="container-fluid p-4">
                ...
                <div class="border-bottom pb-3 mb-3 d-flex align-items-center justify-content-between">
                    <div class="d-flex flex-column gap-1">
                        ...
                        <nav aria-label="breadcrumb">
                            <ol class="breadcrumb">
                            <li class="breadcrumb-item active" aria-current="page">Motto</li>
                            </ol>
                        </nav>
                    </div>
                </div>
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-12">
                <form id="formMotto" alamat="{{ route('tentang.mottoUpdate') }}" data-parsley-validate method="POST" enctype="multipart/form-data">
                    <input type="hidden" name="id" value="{{$data->id_tentang_motto}}">
                    @csrf
                    {{-- {{ csrf_field() }} --}}
                    <div class="card">
                        ...
                        <div class="card-body">
                            <div class="row mb-2 align-items-center">
                                <label for="context" class="col-sm-2 col-form-label">motto</label>
                                <div class="col-sm-10">
                                    <textarea class="form-control" required name="motto" id="context" cols="50" rows="5"
                                        placeholder="Motto Stainaa">{{$data->isi_motto}}</textarea>
                                </div>
                            </div>
                        </div>
    ...
    <script>
        $(document).ready(function() {
            $("#formMotto").on('submit', function(e) {
                e.preventDefault()
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swal.fire({
                                ...
                                $('#spinnerWrapper').css('display', 'flex')
                                window.location.href = "{{ url('/tentangMottoIndex') }}"
                            })
                        }
                    })
                }
