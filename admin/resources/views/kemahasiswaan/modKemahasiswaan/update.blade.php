@extends('template')
@section('title')
    Kemahasiswaan | Mod Edit
@endsection

<?php
    $data = DB::table('landing_kemahasiswaan')->where('id_landing_kemahasiswaan', $id)->first();
?>
...
@section('konten')
    <section class="container-fluid p-4">
        ...
        <div class="row">
            <div class="col-lg-12 col-md-12 col-12">
                <form id="formkemahasiswaan" alamat="{{ route('kemahasiswaan.modUpdate') }}" data-parsley-validate method="POST" enctype="multipart/form-data">
                    @csrf
                    <input type="hidden" name="id" value="{{$data->id_landing_kemahasiswaan}}">
                            ...
                            <div class="row mb-2 align-items-center">
                                ...
                                <div class="col-sm-10">
                                    <div class="form-group">
                                        ...
                                        <input class="form-control" type="file"  name="gambarkemahasiswaan" id="gambarkemahasiswaan"  <?= $gambar?>>
                                    </div>
                                </div>
                            </div>
                        </div>
    ...
    <script>
                    ...
                    $.ajax({
                        ...
                        success: function(response) {
                            ...
                            swal.fire({
                                ...
                                $('#spinnerWrapper').css('display', 'flex')
                                window.location.href = "{{ url('/kemahasiswaanModIndex') }}"
                            })
                        }
                    })
                }
        ...
        $(function () {
            ...
            $("#gambarkemahasiswaan").change(function(){
                if (this.files && this.files[0]) {
                    var size = this.files[0].size;
                    if (size > 100000000) {
                        alert("Maximum file size 1000px exceeds");
                        $("#gambarkemahasiswaan").val("")
                        return;
                    } else {
                        if (this.files && this.files[0]) {
